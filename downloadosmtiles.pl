...
our $usage = qq{Usage: 
   $0 --latitude=d[:d] --longitude=d[:d] --zoom=z[:z] [--quiet] [--baseurl=url] [--destdir=dir] [--dumptilelist=filename] [--thread=number] [--skip]
   $0 --link=url [--latitude=d[:d]] [--longitude=d[:d]] [--zoom=z[:z]] [--quiet] [--baseurl=url] [--destdir=dir] [--dumptilelist=filename] [--thread=number] [--skip]
...
die "$usage\n"
    unless GetOptions(\%opt,
		      ...
		      "skip", "quiet", "baseurl=s", "destdir=s", 
		      "dumptilelist=s", "loadtilelist=s", "thread=s") &&
	   @ARGV == 0;
...
sub downloadtiles
{
    ...
    my $lwpua = LWP::UserAgent::Determined->new;
    $lwpua->env_proxy;
    $lwpua->timeout(600); ### 600 seconds
    $lwpua->timing(10, 20, 20, 20, 30); ### retry 5x
    my $http_codes_hr = $lwpua->codes_to_determinate();
    $http_codes_hr->{400} = 1;
    $http_codes_hr->{404} = 1;
    $http_codes_hr->{504} = 1;
...
sub downloadtile
{
    my ($lwpua, $tilex, $tiley, $zoom, $pm) = @_;
...
Do not download a file if the file already exists on the destination 
folder (name and non zero file size)
...
=head2 C<--thread=number>

Get tiles using <number> of parallel threads (i.e., forks); default: 1 
...
a proxy server.  This is implemented by L<LWP::UserAgent::Determined>.
